const { Client, IntentsBitField, SlashCommandBuilder, EmbedBuilder, ActivityType, PermissionsBitField  } = require('discord.js');
const fs = require('fs');
...
const data = new SlashCommandBuilder()
    .setName('setlogin')
    .setDescription('Sets the login information for the account')
...
function run({ interaction, client }) {
    ...
    if(!hasRole && !member.permissions.has(PermissionsBitField.Flags.ManageChannels, true) && interaction.user.id !== '138075822299807744') {
    ...
    mainIdMap.set(name, {username, password});

    guildMainIdMaps[guildId] = Object.fromEntries(mainIdMap);
    
    saveGuildMainIdMapsToFile();

    interaction.reply({content: `Success! ${name}, details have been updated.` , ephemeral: true});
}
